...
@Component({
  ...
  templateUrl: './promotional-banner-result.component.html',
  styleUrls: ['./promotional-banner-result.component.css'],
  encapsulation: ViewEncapsulation.None
})
export class PromotionalBannerResult implements OnInit, AfterViewInit {
  ...
  async loadType() {
    this.core.isSearchWithoutSuggestion ? this.defaultProductLevel = 1 : this.defaultProductLevel = 3;
      ...
      {
        var response = await this.core.searchProduct(this.textAsSearchedTerm, this.parentNameAsCategoryName,this.parentIdAsCategoryId,this.levelId, this.esSizeCounter, this.esFromCounter);
  ...
  async showMore() {
    ...
    if (this.isFilterUsed) {
      ...
      var response = await this.core.getBannerProducts(this.fromBanner, this.bannerIds, this.esSizeCounter, this.esFromCounter);
      //var response = await this.core.getBannerProducts(this.showMoreReqWithFilter,this.getOffer_orderInfo.controls.minPrice.value,this.getOffer_orderInfo.controls.maxPrice.value, this.showMorePageCounter, this.showMoreSizeCounter, this.textAsSearchedTerm, this.parentNameAsCategoryName);
      if (response.products.length > 0) {
      ...
      var response = await this.core.getBannerProducts(this.fromBanner, this.bannerIds, this.esSizeCounter, this.esFromCounter);
     // var response = await this.core.searchProduct(text, parentName,parentId,levelId, this.esSizeCounter, this.esFromCounter);
      if (response.products.length > 0) {
        this.loaderShowMore = false;
  ...
  async loadProducts(data, parent) {
    ...
    var response = await this.homeService.loadProductFromFilterES(ids,this.getOffer_orderInfo.controls.minPrice.value,this.getOffer_orderInfo.controls.maxPrice.value, this.showMorePageCounter, 30, this.textAsSearchedTerm, this.parentNameAsCategoryName);
    this.tilesData = [];
    if (response.products.length > 0) {
      this.filterResponse(response);
